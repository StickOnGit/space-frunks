...
This is my first attempt at a 'complete' Python project.

This game is written in Python 2.7 and requires PyGame.

CHANGELOG:
09/24/14
    - put objects into their own .py files - sf.py is no longer a monolithic dump truck wasteland of Python objects!
    - a Screen object as a weird view-controller thing - objects have an 'obj.opacity' attribute, but they don't set the alpha on their own surfaces, the Screen object does that. Additionally objects that slowly fade in or out don't change their own opacity over time, the Screen object increments that attribute.
    - Screen.apply_fx() yields a generator, which ideally would work with display.update() to just update the rects that are blitted, but for whatever reason it doesn't work well on Ubuntu (works fine on OS X) so for now the generator object is just iterated over with a "for x in this_obj: pass" statement. meh.
    - sounds are played via a sound manager - just a dict that holds the loaded files, listens for events and tells the pygame mixer to do things
    - 30 FPS instead of 60, because 60 is silly for a game like this.
    - my daughter recognizes her name and is starting to crawl more, but that has little to do with anything. it is the reason I take so bloody long to update things though. <3 janet!
    - a couple things are in "test mode" so the graphics are a little less awesome than before and the hi-score list is disabled, because yeah, n stuff. easy to fix, just... haven't.
...
Not sure if I'll be sharing this beyond my own circle of programming friends, so if you're reading this, maybe you're privileged and you should hug yourself. :)
